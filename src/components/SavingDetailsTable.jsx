...
const SavingDetailsTable = ({ openModal }) => {
  ...
  return (
    <div>
      ...
      <div className="overflow-y-auto border rounded-md shadow-md" style={{ height: "560px" }}>
        ...
        {!loading && !error && data.length > 0 && (
          <table className="min-w-full bg-white">
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={index} className="border-b hover:bg-gray-100 cursor-pointer" onClick={() => openModal(row, row.DiscountedSku)}>
                  <td className="p-3 flex items-center gap-2">
                    {row.ProviderID === 1 ? <img src={awsLogo} alt="AWS" className="w-8 h-8" /> : <img src={azureLogo} alt="Azure" className="w-8 h-8" />}
